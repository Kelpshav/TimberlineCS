...
public class MonsterGrid extends JPanel implements ActionListener {
    ...
    boolean active;
    private int delay = 800; //in ms
    private Timer timer;
    private boolean[] monsters;
    private ImageIcon image;
    private String imagesAvail[] = {"mike.jpg", "sean.jpg", "hank.jpg"};
    ...
    public MonsterGrid(int d, int m){
        ...
        for(int i = 0; i < images.length; i++){
            ...
            images[i] = imagesAvail[i%imagesAvail.length];
		...
		{   
			if (!active) {
                            ...
                            for(int i = 0; i < monsters.length; i++){
                                ...
                                moveMonsters();
                                if(monsters[i] == false){
                                image = new ImageIcon("/home/timberlinepluska/Desktop/APCSA/MonsterMash/src/images/" + images[i]);
                                tiles[monsterX][monsterY].setIcon(image);
                                tiles[monsterX][monsterY].setSelected(true);
                                tiles[monsterX][monsterY].setText(i+"");
